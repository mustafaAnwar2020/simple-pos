...
@section('content')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('dashboard.category.create') }}">
                Create Category
            </a>
        </div>
    </div>
                ...
                <tbody>
                    ...
                    @foreach ($categories as $cat)
                        <tr>
                            ...
                            <td>{{ $cat->name }}</td>
                            <td>
                                <a class="btn btn-sm btn-info" href="{{ route('dashboard.category.edit', $cat) }}">
                                    @lang('site.edit')
                                </a>

                                <form action="{{ route('dashboard.category.destroy', $cat) }}" method="POST"
